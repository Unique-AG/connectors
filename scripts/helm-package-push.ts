...
async function chartExists(registry: string, chartName: string, version: string): Promise<boolean> {
...
async function packageChart(chartPath: string, destination: string): Promise<string> {
  const result = await exec(['helm', 'package', chartPath, '--destination', destination]);
  if (!result.success) {
