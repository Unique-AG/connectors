...
import { mapOutlookMessageToEmailInput } from './lib/map-outlook-message-to-email-input';
...
export class EmailSyncService {
  ...
  public async onFullSyncRequested(event: EmailFullSyncRequestedEvent) {
    await this.syncFolderEmails(event.userProfileId, event.folderId, { forceInitialSync: true });
  }
  ...
  private async syncFolderEmails(
    ...
    folderId: string,
    { forceInitialSync = false }: { forceInitialSync?: boolean } = {},
  ): Promise<void> {
    const folder = await this.db.query.folders.findFirst({
      where: and(
  ...
  private async performDeltaSync(
    ...
    const foldersMap = await this.getFoldersMap();
    ...
    while (hasMorePages) {
      ...
      for (const message of response.value) {
        ...
        if (isDeleted) {
          const deletedEmailIds = await this.emailService.deleteEmails(userProfileId, [
            // biome-ignore lint/style/noNonNullAssertion: All messages always have an id. The MS type is faulty.
            message.id!,
          ]);
          if (!deletedEmailIds.length) throw new Error('Failed to delete email');
          const [deletedEmailId] = deletedEmailIds;
          if (!deletedEmailId) throw new Error('Failed to delete email');

          this.eventEmitter.emit(
            EmailEvents.EmailDeleted,
            new EmailDeletedEvent(userProfileId, deletedEmailId),
          );
        } else {
          const mappedEmail = mapOutlookMessageToEmailInput({
            message,
            userProfileId: userProfileId.toString(),
            folderId: folder.id,
            foldersMap,
          ...
          const emailId = await this.emailService.upsertEmail(
            userProfileId,
            folder.id,
            mappedEmail,
          );
          this.eventEmitter.emit(
            EmailEvents.EmailSaved,
            new EmailSavedEvent(userProfileId, emailId),
          );
        }
  ...
  /**
   * Returns a map of folder ids to names for all folders.
   */
  private async getFoldersMap(): Promise<Map<string, string>> {
