...
The connector runs a **configurable cron job** (default: every 15 minutes) that iterates over all connected users and syncs their OneNote content into the Unique knowledge base.
...
The connector uses **delegated** Microsoft OAuth (per-user tokens), since the OneNote Graph API does not support app-only authentication. This follows the same pattern as the `teams-mcp` service:
...
| `UNIQUE_ROOT_SCOPE_PATH` | `OneNote-Connector` | Root folder path in Unique |
...
```bash
docker build -t onenote-connector .
docker run -p 9543:9543 --env-file .env onenote-connector
```
...
```env
OTEL_SERVICE_NAME=onenote-connector
