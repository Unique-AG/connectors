...
async function bootstrap() {
  ...
  await app.listen(config.port, () =>
    logger.log(
      `OneNote Connector successfully started and listening on http://localhost:${config.port}`,
      'Bootstrap',
    ),
  );
}
...
initOpenTelemetry({
  defaultServiceName: 'onenote-connector',
  defaultServiceVersion: packageJson.version,
  includePgInstrumentation: true,
});
void runWithInstrumentation(bootstrap, 'onenote-connector');
