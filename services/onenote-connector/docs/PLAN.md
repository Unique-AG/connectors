...
overview: Create a new NestJS service `onenote-connector` that periodically synchronizes OneNote notebooks via Microsoft Graph API and ingests their content into the Unique knowledge base, following the architectural patterns of the existing teams-mcp and sharepoint-connector services.
todos:
  - id: scaffold
    content: Scaffold the onenote-connector service (package.json, tsconfig, nest-cli, Dockerfile, .env.example, main.ts, app.module.ts)
    status: in_progress
  - id: config
...
A new NestJS service under `services/onenote-connector/` that combines patterns from:
