import {
  AesGcmEncryptionModule,
  AesGcmEncryptionService,
} from "@unique-ag/aes-gcm-encryption";
  ...
  imports: [
    ...
    McpOAuthModule.forRootAsync({
      ...
      useFactory: async (
        ...
        serverUrl: configService
          .get("app.selfUrl", { infer: true })
          .toString()
          .slice(0, -1),
        resource: new URL(
          "/mcp",
          configService.get("app.selfUrl", { infer: true }),
        ).toString(),

        accessTokenExpiresIn: configService.get(
