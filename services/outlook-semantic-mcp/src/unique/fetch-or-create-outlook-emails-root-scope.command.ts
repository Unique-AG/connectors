...
export const getRootScopePath = (userEmail: string): string => `${userEmail}/Outlook Emails`;
...
export class FetchOrCreateOutlookEmailsRootScopeCommand {
  ...
  public async run(userEmail: string): Promise<Scope> {
    ...
    const [rootScope, toBeProcessedScope] = await this.uniqueScopesService.createScopesBasedOnPaths(
    ...
    assert.ok(toBeProcessedScope, `Could not create to be processed scope for email: ${userEmail}`);
    span?.addEvent(`Root scopes created`);

    if (!rootScope.externalId) {
      span?.addEvent(`Locking root scope with externalId`);
      await this.uniqueScopesService.updateScopeExternalId(rootScope.id, scopePath);
    ...
    if (!toBeProcessedScope.externalId) {
      ...
      await this.uniqueScopesService.updateScopeExternalId(toBeProcessedScope.id, scopePath);
