...
## Outlook Calendar
- Use \`list_calendars\` to see the user's own calendars, calendars shared with them, and calendars of mailboxes they have Full Access to. Pass each \`calendarRef\` to \`search_calendar_events\` as \`calendars\`. Primary calendars (\`isDefaultCalendar\` true) are listed first; holiday and birthday calendars are not primary — for meetings between people, pass only the primary calendars. \`ownerEmail\` on a calendar with \`isOwn\` true is the signed-in user SMTP — use it in \`check_availability\` and \`suggest_meeting_times\` \`attendees\` when they want to attend.
