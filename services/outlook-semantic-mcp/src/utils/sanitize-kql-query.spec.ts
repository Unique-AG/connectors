...
describe('sanitizeKqlQuery', () => {
  describe('smart quote normalization', () => {
    it('replaces Unicode left/right single quotes (‘’) with ASCII apostrophes', () => {
      expect(sanitizeKqlQuery('subject:‘hello world’')).toBe("subject:'hello world'");
    });

    it('replaces Unicode left/right double quotes (“”) with ASCII double quotes', () => {
      expect(sanitizeKqlQuery('subject:“hello world”')).toBe('subject:"hello world"');
    });
  });
  ...
  describe('unsupported property stripping', () => {
    ...
    it('strips isRead: clauses', () => {
      expect(sanitizeKqlQuery('isRead:true from:alice@example.com')).toBe('from:alice@example.com');
    });
  ...
  describe('supported properties are preserved', () => {
    ...
    ])('passes through %s unchanged', (query) => {
      expect(sanitizeKqlQuery(query)).toBe(query);
    });
  });

  describe('compound queries', () => {
    it('preserves valid compound query with multiple supported filters', () => {
    ...
    it('trims leading/trailing whitespace left by stripping', () => {
