...
Prefer rangeType "relative" with a documented range. Weeks start Monday (ISO-8601). lastMonth is the previous calendar month; past30Days is a rolling window. For vague phrasing pick the closest documented range and rely on resolvedWindow.interpretation.
...
eventRef, eventId, calendarId and accessPath are internal — never display them. Pass eventRef verbatim to other calendar tools. webLink is the only user-facing event URL besides joinUrl; if it is empty, render the subject as plain text.
