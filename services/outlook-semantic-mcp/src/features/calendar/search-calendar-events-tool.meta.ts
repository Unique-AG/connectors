...
Sent to Microsoft Graph, so they narrow before the cap: the time window, subject.startsWith, and the first value of categories. Every event returned under these is a real match, and searchNotes tells you when more of the same exist.

Evaluated here, after the cap, on the events Graph already returned: subject.contains, attendees, and any category after the first. These trim what you were given rather than searching the window. Treat them as a convenience. Nothing found under them means "nothing matched in what came back", not "no such meeting exists".
...
Before calling, call list_calendars and pass its calendarRef values as calendars. That is the only way to choose which calendars to search; do not pass a mailbox address. For meetings between people, pass every isDefaultCalendar true calendar and every isOwn false calendar (shared or delegated). Skip holiday and birthday calendars by name — do not skip every isDefaultCalendar false calendar, because accepted shared calendars are also false. Pass every such calendarRef when the user wants all meeting calendars.
...
- subject.startsWith only helps if you know how the title begins. A wrong prefix excludes the event outright, so use subject.contains instead and accept that it runs after the cap.
