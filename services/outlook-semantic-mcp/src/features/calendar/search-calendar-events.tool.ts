...
export const SearchCalendarEventsInputSchema = FiltersSchema.extend({
  calendars: z
    ...
    .max(50)
    .describe(
      'Which calendars to search. Call list_calendars first and pass each calendarRef through unchanged. Prefer calendars with isDefaultCalendar true — those are the primary meeting calendars. Skip holiday, birthday, and other extra calendars (isDefaultCalendar false). Pass every primary calendarRef only when the user wants all meeting calendars. Never assemble a calendarRef yourself and never use a mailbox address to choose calendars.',
    ),
