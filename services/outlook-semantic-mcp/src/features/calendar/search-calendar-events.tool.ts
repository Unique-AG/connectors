  ...
  .union([
    z.strictObject({
      startsWith: z
        ...
        .min(1)
        .describe(
          'How the subject begins. Graph applies this before results are capped. A wrong prefix excludes the event outright, so use contains unless you actually know the start of the title, e.g. "Weekly" for "Weekly Sales Review".',
        ),
    }),
    z.strictObject({
      contains: z
        ...
        .min(1)
        .describe(
          'A word or phrase anywhere in the subject. Graph applies this before results are capped, so prefer it whenever you do not know how the title begins, e.g. "Sales" for "Weekly Sales Review".',
        ),
    }),
  ])
  .describe(
...
const FiltersSchema = z.object({
  ...
  categories: z
    ...
    .optional()
    .describe(
      'Every category listed must be on the event for it to match. To find events carrying either of two categories, search once per category rather than listing both. Graph cannot filter on categories, so this is applied after results are capped: an empty result does not prove the meeting does not exist. Each must name an existing Outlook category exactly, so omit rather than guessing.',
    ),
});
...
export const SearchCalendarEventsInputSchema = FiltersSchema.extend({
  calendars: z
    ...
    .max(50)
    .describe(
      "Which calendars to search. Call list_calendars first and pass each calendarRef through unchanged. For meetings between people, pass every isDefaultCalendar true calendar and every isOwn false calendar (shared). Skip holiday and birthday calendars by name — do not skip every isDefaultCalendar false calendar, because accepted shared calendars are also false. Pass every such calendarRef when the user wants all meeting calendars. When the user asks what meetings another person has, or to look in that person's calendar, pass only the calendarRef whose ownerEmail matches them (isOwn false). If none is listed, do not pass isOwn true calendars as a substitute. Never assemble a calendarRef yourself.",
    ),
...
const CalendarEventSchema = z.object({
  ...
  ownerEmail: z
    ...
    .nullable()
    .describe(
      "SMTP of the calendar this event was read from. Match this to the person whose calendar the user asked about. Null if Graph omitted it. Never present events whose ownerEmail is the signed-in user as another person's calendar.",
    ),
  ownerName: z
    .string()
    .nullable()
    .describe(
      'Display name of the calendar owner this event was read from, or null if Graph omitted it.',
    ),
  isOwn: z
    .boolean()
    .describe(
      "True when this event was read from the signed-in user's own calendar. False when it was read from a calendar shared with them. Never present isOwn true events as another person's calendar.",
    ),
...
export class SearchCalendarEventsTool {
  ...
  @Tool({
    ...
    description:
      'Search Outlook calendar events in a time window. Call list_calendars first and pass those calendarRef values as calendars — that is how you choose among the signed-in user\'s own and shared calendars. For meetings between people, pass every isDefaultCalendar true calendar and every isOwn false calendar (shared). Skip holiday and birthday calendars by name — do not skip every isDefaultCalendar false calendar, because accepted shared calendars are also false. Pass every such calendarRef when the user wants all meeting calendars. When the user asks what meetings another person has, or to look in that person\'s calendar, pass only the calendarRef whose ownerEmail matches them (isOwn false). If none is listed, do not search isOwn true calendars and present those events as theirs — use check_availability for free/busy. organizerName / organizerEmail is who created the meeting, not whose calendar you searched. attendees finds meetings with that person on calendars you can already read; it is not how you open their calendar.\n\nTime window: prefer dateRange.rangeType=relative with a documented range (today, tomorrow, thisWeek, nextWeek, lastMonth, next7Days, …); weeks start Monday. Vague phrasing ("soon", "recently") should use the closest documented range. Absolute startDateTime/endDateTime must include a timezone offset — Graph does not reinterpret them via Prefer: outlook.timezone.\n\nWhere each filter runs, because it changes what an empty result means. Results are capped, and the cap is what makes an answer incomplete. subject (either form) is sent to Microsoft Graph, so it narrows BEFORE the cap: every event you get back is a real match, and searchNotes tells you when more of the same exist. attendees and categories are evaluated in this service AFTER the cap, on the events Graph already returned — so they are a convenience, not a guarantee. They can return nothing while matching events sit outside what was fetched.\n\nSo: an empty result from attendees or categories does not prove the meeting does not exist. When searchNotes reports that results were capped, say the answer may be incomplete and offer a narrower window instead of answering "there are no such meetings".\n\nGather what you need before calling, rather than guessing a filter. attendees is an exact whole-address match, not a name search: a partial or wrong address silently returns nothing that reads exactly like an empty calendar. Resolve a name with lookup_contacts, or ask the user, first. categories must match an existing Outlook category exactly. Use subject.contains unless you actually know how the title begins — both run on Graph, and a wrong prefix excludes the event entirely. attendees and categories are AND filters: every value listed must be on the event, so search once per value when the user means either. If you cannot build a filter confidently, either ask the user or search on the time window alone and read the results.\n\nEach result includes the full plain-text body (possibly truncated — see bodyTruncated); there is no second tool to open an event. eventRef, eventId and calendarId are internal — never display them. If searchNotes is present, show it after the results. If a relative range was used, state resolvedWindow.interpretation. If consentRequired is true, ask the user to reconnect Outlook.',
