...
export const SearchCalendarEventsInputSchema = FiltersSchema.extend({
  calendars: z
    ...
    .max(50)
    .describe(
      'Which calendars to search. Call list_calendars first and pass each calendarRef through unchanged. For meetings between people, pass every isDefaultCalendar true calendar and every isOwn false calendar (shared or delegated). Skip holiday and birthday calendars by name — do not skip every isDefaultCalendar false calendar, because accepted shared calendars are also false. Pass every such calendarRef when the user wants all meeting calendars. Never assemble a calendarRef yourself and never use a mailbox address to choose calendars.',
    ),
