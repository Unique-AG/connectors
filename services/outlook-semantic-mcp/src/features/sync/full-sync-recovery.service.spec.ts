...
function createService({ amqp = createMockAmqp(), db = createMockDb() } = {}) {
  ...
  return new FullSyncRecoveryService(schedulerRegistry as any, amqp as any, db as any);
}
...
describe('FullSyncRecoveryService', () => {
  ...
  describe('publishRetriggerEvents', () => {
    it('does nothing when no retriggerable configs are found', async () => {
      ...
      await service.publishRetriggerEvents();
    ...
    it('does not throw on publish error', async () => {
      ...
      await expect(service.publishRetriggerEvents()).resolves.not.toThrow();
    });
  ...
  describe('lifecycle', () => {
    it('registers cron job on module init', () => {
      const schedulerRegistry = createMockSchedulerRegistry();
      const service = new FullSyncRecoveryService(
