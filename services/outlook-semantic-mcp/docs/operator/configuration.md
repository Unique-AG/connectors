...
| `DEFAULT_MAIL_FILTERS` | `mcpConfig.defaultMailFilters` | (required) | JSON string controlling which emails are synced — see [Mail Filters](#mail-filters). The application has no built-in default; this value must be provided via Helm `values.yaml` or set explicitly when running outside Helm. The `ignoredBefore` field is required — without it, the application will not start. **Warning:** Setting `ignoredBefore` far in the past can cause very large initial syncs — significant time and Microsoft Graph API quota consumption. |
