...
| mcpConfig.app.calendarIntegration | string | `"disabled"` | Outlook calendar tools and Calendars.ReadWrite.Shared scope. Set to "enabled"    to register calendar tools and request the extra Graph scope on OAuth / token refresh.    Also set calendar_integration=true on the Entra Terraform module (admin consent)    before flipping this flag; otherwise mail token refresh fails with invalid_grant.    Env var: CALENDAR_INTEGRATION |
