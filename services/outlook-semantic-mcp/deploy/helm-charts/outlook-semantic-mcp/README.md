...
| mcpConfig.app.calendarIntegration | string | `"disabled"` | Outlook calendar tools and Calendars.ReadWrite.Shared scope. Set to "enabled"    to register calendar tools and request the extra Graph scope on OAuth / token refresh.    Env var: CALENDAR_INTEGRATION |
