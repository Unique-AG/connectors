...
| mcpConfig.app.calendarIntegration | string | `"disabled"` | Outlook calendar tools and Calendars.ReadWrite.Shared scope. Set to "enabled"    to register calendar tools and request the extra Graph scope on OAuth / token refresh.    Existing users must reconnect unless Entra admin consent already covers that scope;    otherwise mail token refresh fails with invalid_grant.    Env var: CALENDAR_INTEGRATION |
