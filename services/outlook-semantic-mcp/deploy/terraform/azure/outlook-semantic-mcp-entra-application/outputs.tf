output "client_id" {
  ...
  value       = azuread_application.outlook_semantic_mcp.client_id
}
...
output "service_principal_object_id" {
  ...
  value       = var.service_principal_configuration != null ? azuread_service_principal.outlook_semantic_mcp[0].object_id : null
}
...
output "admin_consent_url" {
  ...
  value       = "https://login.microsoftonline.com/organizations/v2.0/adminconsent?client_id=${azuread_application.outlook_semantic_mcp.client_id}&scope=https://graph.microsoft.com/.default${var.admin_consent_redirect_uri != null ? "&redirect_uri=${urlencode(var.admin_consent_redirect_uri)}" : ""}"
}
