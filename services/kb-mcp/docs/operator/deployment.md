...
routes:
  ...
  pathPrefix: /   # chart default is /kb-mcp; this keeps the URL at the bare /mcp this guide uses
  auth:
...
`DATABASE_URL` isn't listed here: `postgresql.enabled: true` is the chart default, but
`postgresql.connection` is required either way and ships empty, so it's never actually minimal.
See [PostgreSQL](#PostgreSQL) below for both paths (bring your own CloudNativePG `Cluster`, or an
external instance).

## PostgreSQL

Postgres holds OAuth-proxy state only: client registrations, tokens, and JTI mappings. It never
holds knowledge-base data. `postgresql.enabled: true` is the chart default, and `postgresql.connection`
is required whenever it's on, whichever Postgres you use: the chart neither creates nor owns one.

**In-cluster:** bring your own CloudNativePG `Cluster` and point at it:
...
`host` and `url` do different jobs here: `host` only shapes the network-policy egress rule (same
`toEndpoints` mechanism as `internalServices.dependencies`, see Network Policies below), while
`url` is the actual DSN, generated by your `Cluster` into its own secret, and is what becomes
`DATABASE_URL`.

**External:** point at a managed instance instead by supplying `DATABASE_URL` as a secret env var
and setting `postgresql.enabled: false`, bypassing the `postgresql.connection` contract entirely.
