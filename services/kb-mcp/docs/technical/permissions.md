...
or `read_file` result therefore only ever contains knowledge-base content the calling user could
already open in the web app.
...
`content_tree` caches its response in memory, keyed on `(company_id, user_id)`. Two users never
share a cache entry, and the cache is per pod. It is not a shared store.
