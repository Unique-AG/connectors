...
1. **URL scheme**:
   ...
   - Internal docs use
     ``{UNIQUE_MCP_FRONTEND_BASE_URL}/knowledge-upload/{scopeId}?file={contentId}``
...
SEARCH_SYSTEM_PROMPT = (
    ...
    "search directly. Two exceptions: "
    "(1) The user is asking what documents/files exist somewhere rather than "
    "a substantive question about their content — 'which documents do we "
    "have owned by X', 'what's in the X folder', 'list files under X'. "
    "That's an enumeration request, not a content search: call content_tree "
    "(mode='list' or 'tree') instead of search. "
    "(2) The user asks to restrict the search to a named folder ('only in "
    "X', 'just inside the X folder') rather than merely mentioning one as a "
    "topic, call content_tree(mode='tree') first and pass the folder_id it "
    "prints for that folder as folder_ids. "
...
def file_reference_url(
    ...
    *,
    ...
    """Canonical URL for a whole file (content_tree listings, read_file header).

    Deep link when the frontend origin and a scope id are known, else the
    ``unique://content/{id}`` scheme the Unique platform resolves.
    """
    scope = scope_id_from_metadata(metadata)
    if not scope and owner_id and owner_id.startswith("scope_"):
        scope = owner_id
    if frontend_base_url and scope:
        return frontend_document_url(frontend_base_url, scope, content_id)
    return f"unique://content/{content_id}"
...
def _escape_markdown_link_label(name: str) -> str:
    ...
    Without this, a folder like ``[ORG]`` yields ``[[ORG]/file.pdf](url)`` — the
    first ``]`` closes the label early, the ``(url)`` is lost, and clients show a
    stringified remnant such as ``[ORG]/file.pdf — content_id: …``.
    """
