...
from kb_search.tools.content_tree import (
...
def identity(monkeypatch):
    ...
    monkeypatch.setattr("kb_search.tools.content_tree.get_unique_settings_async", mock)
    return mock
...
@pytest.fixture(autouse=True)
def _reset_cache():
    import kb_search.tools.content_tree as ct_module
...
async def test_mode_tree_calls_render_visible_tree_only():
    ...
    with patch("kb_search.tools.content_tree.ContentTree", return_value=mock_tree):
...
async def test_mode_search_without_query_returns_error_without_calling_service():
    with patch("kb_search.tools.content_tree.ContentTree") as mock_cls:
        result = await content_tree(
            mode="search",
            query=None,
            config=ContentTreeToolConfig(),
...
async def test_cache_reuses_same_content_tree_instance_for_same_identity():
    mock_tree = _make_mock_tree()
    with patch(
        "kb_search.tools.content_tree.ContentTree", return_value=mock_tree
    ) as mock_cls:
...
def test_cache_settings_default_and_env_override(monkeypatch):
    from kb_search.tools.content_tree import _ContentTreeCacheSettings
...
async def test_cache_miss_for_different_identity_constructs_new_instance(identity):
    with patch(
        "kb_search.tools.content_tree.ContentTree",
