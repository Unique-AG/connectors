...
import { ScopeInitializerService } from '../unique-api/scope-initializer.service';
...
export class SharepointSynchronizationService {
  ...
  public constructor(
    ...
    private readonly scopeInitializer: ScopeInitializerService,
  ) {}
  ...
  public async synchronize(): Promise<void> {
    ...
    try {
      // Initialize root scope before scanning if rootScopeName is configured
      // const rootScopeName = this.configService.get('unique.rootScopeName', { infer: true });
      // if (rootScopeName) {
      //   this.logger.log(`Initializing root scope: '${rootScopeName}'`);
      //   await this.scopeInitializer.initialize(rootScopeName);
      // }
