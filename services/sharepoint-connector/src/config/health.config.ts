...
import {
  ...
  DEFAULT_HEALTH_SYNC_SITE_FAILURE_THRESHOLD,
...
export const HealthConfigSchema = z.object({
  syncHistorySize: coercedPositiveIntSchema
    ...
    .describe('Number of recent sync runs to keep in the sliding window for health checks'),
  syncSiteFailureThreshold: z.coerce
    .number()
    .min(0)
    .max(1)
    .prefault(DEFAULT_HEALTH_SYNC_SITE_FAILURE_THRESHOLD)
    .describe(
