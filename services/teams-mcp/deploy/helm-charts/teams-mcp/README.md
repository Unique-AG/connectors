...
| mcpConfig.microsoft.signInTenant | string | `"common"` | Tenant the OAuth login should open. Set a GUID when sign-in must use the Enterprise Application (service principal) in a foreign tenant. Leave as "common" when users choose their directory at login. Independent of clientId/clientSecret; not the tenant that owns the app registration. |
