...
| mcpConfig.microsoft | object | `{"clientId":"{{ fail \"mcpConfig.microsoft.clientId is mandatory. Override in your deployment values.\" }}","signInTenant":"common"}` | Microsoft Graph API configuration |
...
| mcpConfig.microsoft.signInTenant | string | `"common"` | Directory users sign into for OAuth (authorize and token). This is the customer directory that receives the Enterprise Application, not the tenant that owns the app registration. Use a GUID to pin a single-tenant app or a dedicated customer directory. Defaults to "common". Aliases "organizations" and "consumers" are also valid. |
