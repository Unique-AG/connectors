...
| mcpConfig.microsoft | object | `{"clientId":"{{ fail \"mcpConfig.microsoft.clientId is mandatory. Override in your deployment values.\" }}","tenantId":"common"}` | Microsoft Graph API configuration |
...
| mcpConfig.microsoft.tenantId | string | `"common"` | Entra directory for OAuth authorize and token endpoints. Use a tenant GUID to pin a single-tenant app or a specific customer directory. Defaults to "common" (multi-tenant). Aliases "organizations" and "consumers" are also valid. |
