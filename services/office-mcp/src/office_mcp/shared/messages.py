...
* **The sender arrives in two shapes.** A read gives `teamworkUserIdentity`
...
class MessageSender(BaseModel):
    ...
    Two shapes: search hits carry Exchange-style `emailAddress` (name and address), while Teams
    reads return `teamworkUserIdentity` (id and optional name, no email). Which fields are filled
    says which shape Graph answered with. A null is not evidence that the sender has no name or no
    address.
    """
...
def sender_of(identity: ChatMessageFromIdentitySet | None) -> MessageSender | None:
    """The sender, or None when the identity set names nobody.

    One function for both identity shapes so a search hit and a read of the same message report
    the same sender. A null `from` is how Graph sends system event messages.
    """
    if identity is None:
        return None
