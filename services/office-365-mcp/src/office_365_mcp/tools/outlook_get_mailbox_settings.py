"""`outlook_get_mailbox_settings` reports what quietly acts on this mailbox, and what cannot be
seen.

**This is a safety tool before it is a management tool, and its answer carries a gap that must
travel with it.** Exchange mailbox-level forwarding — `Set-Mailbox -ForwardingSmtpAddress`, and
the Forwarding box in the Exchange admin center — is not an inbox rule. It is also not a mailbox
setting that Microsoft Graph publishes. `mailboxSettings` carries exactly `archiveFolder`,
...
(https://learn.microsoft.com/en-us/graph/api/resources/mailboxsettings). No
`forwardingSmtpAddress` is among them, and no other endpoint that this connector can call
carries one either. So "no forwarding rule found" does NOT mean "your mail is not being
forwarded". `covers_mailbox_level_forwarding` is a constant `false` on every response, and
states that fact in plain words. A confident wrong answer here tells a user their mail is
private, while a copy of it leaves the tenant. That is the worst failure that this tool can
produce.
...
from a list of recipients. It must already know which key to look under. The rule that forwards
mail is the whole reason this tool exists. So this tool reads `forwards_to`, `redirects_to`, and
`forward_as_attachment_to` off `actions`, and answers them as plain address lists.

**This tool does not read a rule's conditions.** This answers what a rule *does*, not which mail
it acts on. So a forwarding rule here can forward everything, or one sender's mail. Outlook is
the place to see which. A partial reading of `conditions` and `exceptions` would look like an
answer to a question that this tool did not ask. The same holds for the actions that only sort
mail. This tool does not report `copyToFolder`, `markImportance`, or `assignCategories`.

**`include` exists so that one question costs one request.** The three collections are three
separate Graph calls under one permission. So a caller who wants the automatic reply does not
need a rules listing and a categories listing just to find out. Anything that `include` did not
ask for comes back null, not empty: an empty list reads as "there are none", which is a
different claim entirely.

**The rules are the Inbox folder's rules.** Graph attaches `messageRules` to `mailFolder`, but
documents the collection as "the rules that apply to the user's Inbox folder"
(https://learn.microsoft.com/en-us/graph/api/mailfolder-list-messagerules). So the well-known
name `inbox` is a fixed address, not a folder handle that a caller passes in.

**This tool takes no `mailbox` argument.** Microsoft Graph publishes no `.Shared` variant of
`MailboxSettings.Read`, and Graph refuses this call for any mailbox but the caller's own.
"""
...
# One permission covers all three collections: Microsoft documents `MailboxSettings.Read` as
# the delegated permission for the rule listing, for `mailboxSettings`, and for the master
# categories.
GRAPH_PERMISSIONS: tuple[str, ...] = ("MailboxSettings.Read",)

# This uses `rules`, not the default, so the example is exactly one Graph call. The refusal it
# produces is the rules listing's refusal, not whichever of the three ran first.
...
# verbatim. That advice cannot apply here: this tool takes no id at all.
GRAPH_NOT_FOUND = (
...
# These are bounds on the two collections, far above what Exchange holds in practice. A size
# quota well under this caps a mailbox's rules, and categories form a hand-made list.
MAX_RULES = 200
MAX_CATEGORIES = 500

# This is Graph's locale-independent well-known name for the Inbox, which reaches the Inbox in a
# mailbox of any language. The rules collection exists on this one folder.
_INBOX_FOLDER = "inbox"

# Every property that the answer reads. `conditions` and `exceptions` are absent on purpose —
...
# This is the one property of nine that this tool reports. Microsoft documents that
# `mailboxSettings` requires `$select` to retrieve it. Asking for the rest returns a
# working-hours block that nothing here reports.
_SETTINGS_FIELDS: tuple[str, ...] = ("automaticRepliesSetting",)

_CATEGORY_FIELDS: tuple[str, ...] = ("displayName",)

# These are bound with `=`, not aliased with `type`. These names serve as the query parameters'
# constructor, and also as `RequestConfiguration`'s argument. A `TypeAliasType` is not callable.
...
# These are Microsoft's own spellings, so a value here matches the documentation and the admin
# center, rather than a vocabulary invented in this file.
type AutoReplyStatus = Literal["disabled", "alwaysEnabled", "scheduled"]
type ExternalAudience = Literal["none", "contactsOnly", "all"]

_DESCRIPTION = """\
This tool shows what quietly acts on the signed-in user's mailbox: the inbox rules, the \
automatic reply, and the categories. Use it for "is something forwarding my mail?", "am I \
still out of office?", and "which rules move my mail".

Notes:
- This tool cannot see Exchange mailbox-level forwarding, which Microsoft Graph does not \
publish anywhere. An empty rule list, or rules that forward nothing, does not prove that \
nobody forwards this mailbox's mail.
- This tool reports what each rule does, never which mail it targets. A forwarding rule can \
act on everything, or on one sender's mail. Outlook shows which.
...
class InboxRule(BaseModel):
    ...
    This tool does not report the conditions that trigger the rule, so a rule here can act on
    every message, or on one sender's. Outlook is where that is visible.
    """
    ...
    display_name: str | None = Field(
        description=(
            ...
            + "rule called `Newsletters` can still forward mail out of the organization. This "
    ...
    is_enabled: bool | None = Field(
        description=(
            ...
            + "forwarding rule too. This field is null when Graph did not say."
        )
    )
    sequence: int | None = Field(
        description=(
            "The order that Outlook evaluates rules in, lowest first. This field is null when "
            + "Graph did not say."
        )
    )
    is_read_only: bool | None = Field(
        description=(
            "True for a rule that the rules API cannot modify, usually one written by another "
            + "client or an administrator. It still runs. This field is null when Graph did "
            + "not say."
        )
    )
    has_error: bool | None = Field(
        description=(
            "True when Microsoft 365 marked the rule broken, commonly because it names a "
            + "folder or an address that no longer exists. A rule in error can do only part of "
            + "what its actions describe. This field is null when Graph did not say."
        )
    )
    forwards_to: list[str] = Field(
        description=(
            "Addresses that this rule forwards a copy of the message to — the field that this "
            + "tool exists for. An address outside the user's own domain means copies of their "
            + "mail leave the organization automatically. Each entry is the SMTP address that "
            + "Graph recorded, or the display name when Graph recorded none. An empty list "
            + "means this rule forwards nothing. It says nothing about other rules, or about "
            + "mailbox-level forwarding, which this tool cannot see at all."
        )
    )
    redirects_to: list[str] = Field(
        description=(
            "Addresses that this rule redirects the message to. A redirect keeps the original "
            + "sender, so a reply goes back to them, not to this user. This is harder to "
    ...
    forward_as_attachment_to: list[str] = Field(
        description=(
            "Addresses that this rule forwards the message to as an attachment, headers "
            + "included. Read it the same way as `forwards_to`."
        )
    )
    moves_to_folder: str | None = Field(
        description=(
            "The Graph id of the folder that this rule moves the message to. This field is "
            + "null when the rule moves nothing. The id is opaque on its own: "
            + "outlook_browse_folders reports the id and the name together. Browse there to "
            + "identify it. A rule that files mail out of the Inbox is why a message can look "
            + "like it never arrived."
        )
    )
    deletes: bool | None = Field(
        description=(
            "True when the rule deletes the message. It can move the message to Deleted "
            + "Items, where the user can still find it. Or it can delete the message "
            + "permanently, where nobody can find it again. This field does not tell the two "
            + "apart. It is null when Graph reported neither."
        )
    )
    marks_as_read: bool | None = Field(
        description=(
            "True when the rule marks the message read on arrival, which is how mail can "
            + "arrive already read and unnoticed. This field is null when Graph did not say."
        )
    )
    stops_processing_more_rules: bool | None = Field(
        description=(
            "True when Outlook never evaluates a rule with a higher `sequence` for a message "
            + "that this one matched. So a rule further down the list can look active, yet "
            + "never run for mail that this one already handled. This field is null when Graph "
            + "did not say."
        )
    )
...
class ScheduledMoment(BaseModel):
    """One end of an automatic reply's schedule, as the date and the zone that Graph reports it
    in."""

    date_time: str | None = Field(
        description=(
            "The moment, in Graph's combined `{date}T{time}` spelling, with no offset. Read it "
            + "together with `time_zone`. This field is null when Graph recorded none."
        )
    )
    time_zone: str | None = Field(
        description=(
            "The zone that `date_time` is expressed in, usually `UTC`. Never assume the zone. "
            + "A schedule read in the wrong zone reports an expired auto-reply as still live."
        )
    )
...
class AutomaticReply(BaseModel):
    ...
    status: AutoReplyStatus | None = Field(
        description=(
            "`disabled` sends nothing. `alwaysEnabled` answers every incoming message with no "
            + "end date. `scheduled` answers only between `scheduled_start` and "
            + "`scheduled_end`. This field is null when Graph reported none. Any value other "
            + "than `disabled` means the reply text below reaches people right now."
        )
    )
    external_audience: ExternalAudience | None = Field(
        description=(
            "Who outside this organization receives `external_reply_message`: `none` for "
            + "nobody (colleagues only), `contactsOnly` for senders in the user's contacts, or "
            + "`all` for every outside sender, including strangers and spam. This field is "
            + "null when Graph did not say."
        )
    )
    scheduled_start: ScheduledMoment | None = Field(
        description=(
            "When a `scheduled` reply starts. It has no meaning unless `status` is "
            + "`scheduled`. A past date on a `disabled` reply is leftover, not evidence."
        )
    )
    scheduled_end: ScheduledMoment | None = Field(
        description=(
            "When a `scheduled` reply stops, on the same terms as `scheduled_start`. An "
            + "`alwaysEnabled` reply has no end, whatever this field says."
        )
    )
    internal_reply_message: str | None = Field(
        description=(
            "The reply sent to senders inside this organization, as Graph stored it — usually "
            + "HTML rather than plain text. This field is null when none is set."
        )
    )
    external_reply_message: str | None = Field(
        description=(
            "The reply sent to senders outside this organization, subject to "
            + "`external_audience`. Read it for what it gives away, such as travel dates, a "
            + "deputy's address, or a phone number. This matters beyond whether a reply is on. "
            + "This field is null when none is set."
        )
    )

    @classmethod
    def from_setting(cls, setting: AutomaticRepliesSetting | None) -> Self:
        """Always a reply object, once replies were asked for. A mailbox that Graph reported no
        setting for answers with every field null. That means "Microsoft said nothing", not
        "there is none"."""
        if setting is None:
            return cls(
                status=None,
...
class MailboxSettingsReport(BaseModel):
    """What acts on this mailbox, and — as a field, not a caveat — what is not visible from here
    at all."""
    ...
    covers_mailbox_level_forwarding: Literal[False] = Field(
        ...
        description=(
            ...
            + "`mailboxSettings` has no forwarding property. So an empty `rules` list, or "
            + "rules that forward nothing, does not prove that nobody forwards this mailbox's "
            + "mail. Say that to the user. Do not reassure them. To rule it out, use the "
    ...
    rules: list[InboxRule] | None = Field(
        description=(
            "The Inbox rules, in the order that Graph returned them. Read `sequence` for the "
            + "order they run in. This field is null when `include` did not ask for them. An "
            + "empty list means no inbox rules exist. Before you treat an empty list as "
            + "nothing that touches this mail, see `covers_mailbox_level_forwarding` too."
        )
    )
    rules_capped: bool | None = Field(
        description=(
            f"True when more than {MAX_RULES} rules were on offer and the listing stopped, so "
            + "`rules` is missing some. This is practically always false, because Exchange "
            + "caps a mailbox's rules well below that number. This field is null when "
            + "`include` did not ask for rules."
        )
    )
    automatic_reply: AutomaticReply | None = Field(
        description=(
            "The automatic reply, present whenever `include` asked for it — including for a "
            + "mailbox that Graph reported no setting for, whose fields are then all null. A "
            + "null value here means only that `include` did not ask for it."
        )
    )
    categories: list[str] | None = Field(
        description=(
            "The display names of the categories that this mailbox can tag mail with — free "
            + "strings with no fixed vocabulary. So a category named `Confidential` or `Done` "
            + "means whatever its owner meant by it. This field is null when `include` did not "
            + "ask for them. An empty list means the mailbox has none."
        )
    )
    categories_capped: bool | None = Field(
        description=(
            f"True when more than {MAX_CATEGORIES} categories were on offer and the listing "
            + "stopped, so `categories` is missing some. This field is null when `include` did "
            + "not ask for them."
        )
    )
...
async def get_mailbox_settings(
    ...
    return MailboxSettingsReport(
        ...
        # This is built from `wants_replies`, not from `setting`, which is None for a mailbox
        # that Graph reported no automatic reply for. That is not the same answer as "never
        # asked".
...
def _addresses(recipients: list[Recipient] | None) -> list[str]:
    ...
    recipient. A destination that this tool leaves out is a destination the user does not know
    about.
    """
    named: list[str] = []
...
def _deletes(actions: MessageRuleActions | None) -> bool | None:
    ...
    This function folds the two together, because both answer "does this rule delete my mail"
    with yes. A field that reports only `delete` would answer a rule that permanently deletes
    with silence.
    """
    if actions is None:
        return None
        ...
        include: Annotated[
            ...
            Field(
                description=(
                    ...
                    + "`categories` reads only the category names. Whatever the caller does "
                    + "not ask for comes back null, never empty."
                )
            ),
        ] = "all",
