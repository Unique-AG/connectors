...
class TestTheSpoolFileNeverOutlivesTheBlock:
    ...
    untidiness — it is a pod that gets evicted after enough exports. Every exit from this context
    ...
    async def test_the_directory_is_empty_after_the_download_was_cancelled(
        ...
        """An MCP client hanging up mid-export cancels the task, and `CancelledError` inherits from
        `BaseException` rather than `Exception` — which is exactly the class of unwinding that a
