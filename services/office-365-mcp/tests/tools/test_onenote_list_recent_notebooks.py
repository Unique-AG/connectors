...
class TestWhatItAnswers:
    ...
    @pytest.mark.parametrize(
        "value", ["Unknown", "OneDrive", "OneDriveForBusiness", "OnPremOneDriveForBusiness"]
    )
    async def test_every_sdk_source_service_value_is_mapped(
        self, client: GraphServiceClient, graph: respx.MockRouter, value: str
    ) -> None:
        _ = graph.get(_TRUE_PATH).mock(
            return_value=httpx.Response(200, json={"value": [_recent(source_service=value)]})
        )

        answer = await _list(client)

        assert answer.notebooks[0].source_service == value
...
class TestHowItDeclaresItself:
    ...
    def test_the_call_example_takes_no_arguments(self) -> None:
        assert dict(lister.GRAPH_CALL_EXAMPLE) == {}

    async def test_it_takes_one_argument_and_no_others(self, transport: httpx.AsyncClient) -> None:
        parameters, _tool = await _registered(transport)
        properties = cast("Mapping[str, object]", parameters["properties"])
        assert set(properties) == {"include_personal_notebooks"}
