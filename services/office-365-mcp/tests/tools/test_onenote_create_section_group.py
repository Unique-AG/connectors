...
def _group_payload(
    *,
    group_id: str | None = _CHILD_GROUP_ID,
    name: str | None = "Synthetic section group",
    created_at: str | None = "2026-03-01T09:00:00Z",
) -> dict[str, object]:
    return {"id": group_id, "displayName": name, "createdDateTime": created_at}
...
def notebook_groups(graph: respx.MockRouter) -> respx.Route:
    return graph.post(_NOTEBOOK_GROUPS_PATH).mock(
...
async def _refuses(question: str, about: str) -> str | None:
    assert question
    assert about
    return "No section group was created."
...
) -> creator.CreatedSectionGroup:
    answer = await create_section_group(client, parent=parent, name=name, confirm=confirm)
    assert isinstance(answer, creator.CreatedSectionGroup), (
        "this call was answered with a question, not a section group"
    )
    return answer
...
class TestWhatItSendsToGraph:
    ...
    async def test_a_notebook_parent_posts_via_the_generated_route(
        self, client: GraphServiceClient, notebook_groups: respx.Route, group_groups: respx.Route
    ) -> None:
    ...
    async def test_a_group_parent_posts_via_the_hand_built_route(
        self, client: GraphServiceClient, notebook_groups: respx.Route, group_groups: respx.Route
    ) -> None:
    ...
    @pytest.mark.usefixtures("notebook_audience")
    async def test_the_notebook_route_body_carries_the_display_name(
        self, client: GraphServiceClient, notebook_groups: respx.Route
    ) -> None:
        _ = await _create(client, parent=_NOTEBOOK, name="A new group")

        assert _sent(notebook_groups)["displayName"] == "A new group"

    @pytest.mark.usefixtures("group_audience")
    async def test_the_hand_built_route_body_carries_the_display_name(
        self, client: GraphServiceClient, group_groups: respx.Route
    ) -> None:
        _ = await _create(client, parent=_GROUP, name="A nested group")

        assert _sent(group_groups)["displayName"] == "A nested group"

    @pytest.mark.usefixtures("notebook_audience")
    async def test_the_notebook_route_content_type_is_json(
        self, client: GraphServiceClient, notebook_groups: respx.Route
    ) -> None:
        _ = await _create(client, parent=_NOTEBOOK)

        assert notebook_groups.calls.last.request.headers["content-type"] == "application/json"

    @pytest.mark.usefixtures("group_audience")
    async def test_the_hand_built_route_content_type_is_json(
        self, client: GraphServiceClient, group_groups: respx.Route
    ) -> None:
        _ = await _create(client, parent=_GROUP)

        assert group_groups.calls.last.request.headers["content-type"] == "application/json"

    @pytest.mark.usefixtures("group_audience")
    async def test_the_hand_built_route_asks_for_json_back(
        self, client: GraphServiceClient, group_groups: respx.Route
    ) -> None:
        _ = await _create(client, parent=_GROUP)

        assert group_groups.calls.last.request.headers["accept"] == "application/json"

    @pytest.mark.usefixtures("retry_sleeps", "notebook_audience")
    async def test_the_notebook_route_is_never_retried(
        self, client: GraphServiceClient, graph: respx.MockRouter
    ) -> None:
        post = graph.post(_NOTEBOOK_GROUPS_PATH).mock(return_value=httpx.Response(503))
    ...
    @pytest.mark.usefixtures("retry_sleeps", "group_audience")
    async def test_the_hand_built_route_is_never_retried(
        self, client: GraphServiceClient, graph: respx.MockRouter
    ) -> None:
        post = graph.post(_GROUP_GROUPS_PATH).mock(return_value=httpx.Response(503))
...
class TestWhatItRefuses:
    @pytest.mark.parametrize(
        ...
        [
            ...
            _NOTEBOOK_ID,
            "onenote:///notebooks/",
            OnenoteSectionHandle("1-SYNTHETICSECTION0001!0001").uri,
...
class TestWhatItAnswers:
    @pytest.mark.usefixtures("notebook_audience")
    async def test_the_notebook_route_answer_carries_the_new_groups_handle_and_fields(
        self, client: GraphServiceClient, notebook_groups: respx.Route
    ) -> None:
        notebook_groups.mock(return_value=httpx.Response(201, json=_group_payload(name="Stored")))
    ...
    @pytest.mark.usefixtures("group_audience")
    async def test_the_hand_built_route_answer_carries_the_new_groups_handle_and_fields(
        self, client: GraphServiceClient, group_groups: respx.Route
    ) -> None:
        group_groups.mock(return_value=httpx.Response(201, json=_group_payload(name="Nested")))

        answer = await _create(client, parent=_GROUP)

        assert answer.uri == OnenoteSectionGroupHandle(_CHILD_GROUP_ID).uri
        assert answer.name == "Nested"
    ...
    async def test_the_answer_is_read_off_graph_and_never_echoes_the_argument(
        ...
        notebook_groups.mock(
            return_value=httpx.Response(201, json=_group_payload(name="Untouched by argument"))
        )
    ...
    async def test_a_create_that_names_no_id_is_a_programming_error(
        ...
        notebook_groups.mock(return_value=httpx.Response(201, json=_group_payload(group_id=None)))

        with pytest.raises(AssertionError):
            _ = await _create(client, parent=_NOTEBOOK)

    @pytest.mark.usefixtures("group_audience")
    async def test_a_hand_built_create_that_names_no_id_is_a_programming_error(
        self, client: GraphServiceClient, group_groups: respx.Route
    ) -> None:
        group_groups.mock(return_value=httpx.Response(201, json=_group_payload(group_id=None)))
...
class TestGraphFailures:
    ...
    @pytest.mark.usefixtures("notebook_audience")
    async def test_a_403_on_the_notebook_route_is_a_forbidden(
        self, client: GraphServiceClient, notebook_groups: respx.Route
    ) -> None:
        notebook_groups.mock(
            return_value=httpx.Response(
    ...
    @pytest.mark.usefixtures("group_audience")
    async def test_a_403_on_the_hand_built_route_is_a_forbidden(
        self, client: GraphServiceClient, group_groups: respx.Route
    ) -> None:
        group_groups.mock(
            return_value=httpx.Response(
...
class TestThePersonBetweenTheCreateAndTheOthersInTheNotebook:
    ...
    async def test_the_question_names_the_section_group_and_the_notebook(
        ...
        _ = await _create(
            client, parent=_NOTEBOOK, name="Synthetic section group", confirm=capturing
        )

        assert len(asked) == 1
        assert "Synthetic section group" in asked[0]
        assert "Work" in asked[0]
...
class TestNoRefusalIsEverRaisedByThePrompt:
    ...
    async def test_no_refusal_is_ever_raised(self, answer: object) -> None:
        ...
        refusal = await confirm(
            "Create the section group 'X' in the notebook 'Y'?", "synthetic-state"
        )

        assert isinstance(refusal, str)
        assert refusal.startswith("No section group was created.")
