...
class _CountedBody(httpx.AsyncByteStream):
    ...
    `httpx.Response(content=...)` computes `Content-Length` from the bytes, which is exactly what
    a real server does and exactly what makes it useless for testing a header that lies, is
    missing, or describes bytes that never arrive. A stream lets the header be stated separately
    from the body, and counting the reads is how "refused before the download" is a claim with
    evidence rather than a claim about which line ran first.
    """
...
def _exports_declaring(
    graph: respx.MockRouter, body: _CountedBody, declared: int | None
) -> respx.Route:
    """The same export, with the length Graph declares said separately from what it sends."""
    headers = {"Content-Type": "application/octet-stream"}
    if declared is not None:
        headers["Content-Length"] = str(declared)
    return graph.get(_PATH).mock(return_value=httpx.Response(200, stream=body, headers=headers))
...
class TestWhatItAsksGraphFor:
    async def test_it_reads_the_messages_own_mime_rather_than_its_properties(
        ...
        _ = await export_mail(client, transport, handle=_HANDLE)
    ...
    async def test_it_carries_the_callers_bearer_token_on_the_request_it_issues_itself(
        self,
        client: GraphServiceClient,
        transport: httpx.AsyncClient,
        graph: respx.MockRouter,
    ) -> None:
        """This tool does not go through `send_primitive_async`. It builds the request with
        `to_get_request_information`, hands it to `convert_to_native_async` and sends it on the
        transport itself, so it is the only tool here whose authentication is not the SDK's problem.
        A token dropped on that path is a 401 from Graph in production and nothing else in this
        suite would see it.
        """
        route = _exports(graph)
    ...
    async def test_it_asks_graph_not_to_encode_the_body_it_is_about_to_measure(
        self,
        client: GraphServiceClient,
        transport: httpx.AsyncClient,
        graph: respx.MockRouter,
    ) -> None:
        """The shared transport advertises `gzip, deflate`. Under a coding, `Content-Length` counts
        compressed bytes, so the header guard would measure something that is not the MIME and the
        refusal would report that figure as bytes of MIME. Bandwidth is the price of the sentence
        being true.
        """
        route = _exports(graph)
...
class TestWhatItReturns:
    async def test_it_hands_back_the_bytes_graph_sent_unchanged(
        ...
        exported = await export_mail(client, transport, handle=_HANDLE)
...
class TestWhenItRefuses:
    ...
    async def test_it_refuses_on_the_declared_length_before_reading_a_body(
        self,
        client: GraphServiceClient,
        transport: httpx.AsyncClient,
        graph: respx.MockRouter,
    ) -> None:
        """The whole point of streaming this. A 150 MB message is refused on a header, rather than
        after this process has held every byte of it."""
        body = _CountedBody(b"never read")
        _ = _exports_declaring(graph, body, declared=150 * 1024 * 1024)

        with pytest.raises(ToolError) as refusal:
            _ = await export_mail(client, transport, handle=_HANDLE)

        assert body.reads == 0, "the ceiling fired against the header, not against the bytes"
        assert str(MAX_EXPORT_BYTES) in str(refusal.value)
        assert "outlook_read_mail" in str(refusal.value)
        assert "web_link" in str(refusal.value)

    async def test_it_refuses_bytes_that_cross_the_ceiling_without_a_length_to_warn_it(
        self,
        client: GraphServiceClient,
        transport: httpx.AsyncClient,
        graph: respx.MockRouter,
    ) -> None:
        """A declared length is the server's claim rather than a measurement, and a chunked
        response carries none at all. The second guard is the one that cannot be lied to."""
        body = _CountedBody(b"x" * (MAX_EXPORT_BYTES + 1))
        _ = _exports_declaring(graph, body, declared=None)

        with pytest.raises(ToolError) as refusal:
            _ = await export_mail(client, transport, handle=_HANDLE)

        assert body.reads == 1, "there was no header to refuse, so the bytes had to be counted"
        assert str(MAX_EXPORT_BYTES) in str(refusal.value)
        assert "outlook_read_mail" in str(refusal.value)
        assert "web_link" in str(refusal.value)

    async def test_a_body_that_stops_short_of_its_declared_length_is_refused_as_truncated(
        self,
        client: GraphServiceClient,
        transport: httpx.AsyncClient,
        graph: respx.MockRouter,
    ) -> None:
        """A short `$value` is not a small message: it is the same message with its last
        attachment missing, and it opens. This arrives as `GraphUnavailable` on purpose — the
        shared wording for that is "retry once", which is the right advice for a stream that ended
        early."""
        _ = _exports_declaring(graph, _CountedBody(_MIME), declared=len(_MIME) + 4096)
