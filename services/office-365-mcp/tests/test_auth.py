...
class TestWhichProviderATenantGets:
    def test_one_tenant_keeps_fastmcps_provider_untouched(self) -> None:
        ...
        assert validator.issuer == _issuer(_TENANT_ID)
...
class TestWhatBindsATokenToItsTenant:
    def test_a_token_whose_issuer_is_its_own_tenant_names_that_tenant(self) -> None:
        assert home_tenant({"tid": _TENANT_ID, "iss": _issuer(_TENANT_ID)}) == _TENANT_ID

    @pytest.mark.parametrize(
        "claims",
        [
            pytest.param({"tid": _TENANT_ID, "iss": _issuer(_ANOTHER_TENANT_ID)}, id="iss-another"),
            pytest.param({"tid": _TENANT_ID, "iss": _issuer(ORGANIZATIONS)}, id="iss-literal"),
            pytest.param(
                {"tid": _TENANT_ID.upper(), "iss": _issuer(_TENANT_ID)}, id="tid-case-differs"
            ),
            pytest.param({"iss": _issuer(_TENANT_ID)}, id="no-tid"),
            pytest.param({"tid": _TENANT_ID}, id="no-iss"),
            pytest.param({"tid": "", "iss": _issuer("")}, id="tid-empty"),
            pytest.param(
                {"tid": _TENANT_ID, "iss": f"https://sts.windows.net/{_TENANT_ID}/"}, id="v1-issuer"
            ),
            pytest.param(
                {"tid": _TENANT_ID, "iss": f"{_issuer(_TENANT_ID)}/extra"}, id="iss-suffixed"
            ),
            pytest.param(
                {"tid": _TENANT_ID, "iss": f"{_issuer(_TENANT_ID)}?tenant=other"}, id="iss-query"
            ),
            pytest.param(
                {"tid": _TENANT_ID, "iss": _issuer(_TENANT_ID).replace("https", "HTTPS", 1)},
                id="iss-upper-scheme",
            ),
            pytest.param(
                {
                    "tid": _TENANT_ID,
                    "iss": f"https://login.microsoftonline.com@evil.example/{_TENANT_ID}/v2.0",
                },
                id="iss-authority-is-userinfo",
            ),
        ],
    )
...
class TestTenantBoundTokens:
    async def test_a_token_from_any_tenant_is_accepted_when_its_issuer_is_that_tenant(
        ...
        for tenant_id in (_TENANT_ID, _ANOTHER_TENANT_ID):
            token = _entra_token(private_pem, tid=tenant_id, iss=_issuer(tenant_id))
    ...
    @pytest.mark.parametrize(
        "overrides",
        [
            pytest.param({"iss": _issuer(_ANOTHER_TENANT_ID)}, id="another-tenants-issuer"),
            pytest.param({"iss": _issuer(ORGANIZATIONS)}, id="the-organizations-literal"),
            pytest.param({"tid": "not-a-guid"}, id="tid-is-not-the-issuers"),
...
def _assertion_from(tenant_id: str) -> str:
    return _unsigned_jwt({"tid": tenant_id, "iss": _issuer(tenant_id), "sub": "somebody"})
...
class TestOnBehalfOfAgainstTheTokensOwnTenant:
    ...
    async def test_a_token_that_binds_no_tenant_is_never_exchanged(
        ...
        provider = _build_auth(_entra_config(ORGANIZATIONS))
        assertion = _unsigned_jwt({"tid": _TENANT_ID, "iss": _issuer(ORGANIZATIONS)})
