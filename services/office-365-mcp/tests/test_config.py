...
class TestPublicBaseUrl:
    ...
    def test_production_rejects_a_cleartext_url(self) -> None:
        ...
        with pytest.raises(ValueError, match="https"):
            AppConfig.model_validate(
                {"app_env": AppEnv.PRODUCTION, "public_base_url": "http://office-365-mcp.example"}
            )

    def test_a_cleartext_url_is_allowed_outside_production(self) -> None:
        config = AppConfig.model_validate(
            {"app_env": AppEnv.DEVELOPMENT, "public_base_url": "http://office-365-mcp.example"}
        )

        assert config.issuer == "http://office-365-mcp.example"
    ...
    def test_production_accepts_a_real_public_url(self) -> None:
        config = AppConfig.model_validate(
            {"app_env": AppEnv.PRODUCTION, "public_base_url": "https://office-365-mcp.example"}
        )

        assert config.issuer == "https://office-365-mcp.example"
    ...
    def test_the_parsed_url_is_available_for_host_and_scheme_checks(self) -> None:
        ...
        assert config.public_base_url.host == "office-365-mcp.example"
...
class TestCaseInsensitiveEnumFields:
    ...
    def test_app_env_accepts_mixed_case(self) -> None:
        config = AppConfig.model_validate(
            {"app_env": "Production", "public_base_url": "https://office-365-mcp.example"}
        )
