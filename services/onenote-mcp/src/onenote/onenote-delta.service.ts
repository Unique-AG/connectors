    ...
    this.logger.log(
      {
        ...
        changedNotebookNames: [...changedNotebookNames],
      },
      'Completed delta fetch',
    );

    return { changedNotebookNames, isFullSync, nextDeltaLink };
  }

  public async commitDeltaLink(userProfileId: string, deltaLink: string): Promise<void> {
    this.logger.debug({ userProfileId }, 'Committing delta link after successful sync');
    await this.persistDeltaLink(userProfileId, deltaLink, 'success');
  }
