...
export class OneNoteGraphService {
  ...
  private sanitizeOneNoteName(name: string): string {
  ...
  public async createNotebook(client: Client, displayName: string): Promise<Notebook> {
    const sanitized = this.sanitizeOneNoteName(displayName);
    const response = await client.api('/me/onenote/notebooks').post({ displayName: sanitized });
  ...
  public async createPage(
    ...
    const escapedTitle = this.escapeXhtml(this.sanitizeOneNoteName(title));
