...
const syncServiceHolder: { ref: OneNoteSyncService | null } = { ref: null };
  ...
  imports: [
    ...
    McpOAuthModule.forRootAsync({
      ...
      useFactory: async (
        ...
        oauthStore: new McpOAuthStore(drizzle, aesService, cacheManager, (userProfileId) => {
          if (syncServiceHolder.ref) {
            syncServiceHolder.ref.syncUser(userProfileId).catch((error) => {
              const normalized = normalizeError(error);
              new Logger('AppModule').error(
                { userProfileId, errorMessage: normalized.message, errorName: normalized.name },
                'On-connect sync failed for user',
              );
            });
          }
        }),
...
export class AppModule implements OnModuleInit {
  ...
  onModuleInit(): void {
    syncServiceHolder.ref = this.moduleRef.get(OneNoteSyncService, { strict: false });
  }
}
