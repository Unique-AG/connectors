...
export class McpOAuthStore implements IOAuthStore {
  ...
  public constructor(
    ...
    private readonly onUserConnected?: (userProfileId: string) => void,
  ) {}
  ...
  public async upsertUserProfile(user: PassportUser): Promise<string> {
    ...
    this.logger.log({ userProfileId: saved.id }, 'User profile upserted, triggering initial sync');
    this.onUserConnected?.(saved.id);
