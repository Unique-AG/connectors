...
variable "secrets_placeholders" {
  ...
  default = {
    /**
      ...
      - outlook-fat-mcp-client-secret: The Microsoft Entra application client secret (MICROSOFT_CLIENT_SECRET)

      Database Configuration (database.config.ts):
      - outlook-fat-mcp-database-url: PostgreSQL connection URL (DATABASE_URL)

      AMQP Configuration (amqp.config.ts):
      - outlook-fat-mcp-amqp-url: AMQP/RabbitMQ connection URL (AMQP_URL)
    */
    outlook-fat-mcp-client-secret = { create = true, expiration_date = "2099-12-31T23:59:59Z" }
    outlook-fat-mcp-database-url  = { create = true, expiration_date = "2099-12-31T23:59:59Z" }
    outlook-fat-mcp-amqp-url      = { create = true, expiration_date = "2099-12-31T23:59:59Z" }
  }
}
...
variable "secrets_to_create" {
  ...
  default = {
    ...
    */
    hex_hmac_secret    = { create = true, name = "outlook-fat-mcp-hmac-secret", content_type = "text/hex", rotation_counter = 0, expiration_date = "2099-12-31T23:59:59Z" }
    hex_webhook_secret = { create = true, name = "outlook-fat-mcp-webhook-secret", content_type = "text/hex", rotation_counter = 0, expiration_date = "2099-12-31T23:59:59Z" }
    hex_encryption_key = { create = true, name = "outlook-fat-mcp-encryption-key", content_type = "text/hex", rotation_counter = 0, expiration_date = "2099-12-31T23:59:59Z" }
  }
}
