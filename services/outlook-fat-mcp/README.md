...
| `UNIQUE_ROOT_SCOPE_PATH` | `Outlook-Fat-MCP` | Root folder path in Unique |
...
Services:
- `outlook-fat-mcp`: Main application (port 3000)
- `outlook-fat-mcp-migration`: Database migration runner
...
```bash
helm install outlook-fat-mcp ./deploy/helm-charts/outlook-fat-mcp \
...
- `outlook-fat-mcp-secrets`: Azure Key Vault integration
- `outlook-fat-mcp-entra-application`: Microsoft Entra app registration
...
```env
OTEL_SERVICE_NAME=outlook-fat-mcp
