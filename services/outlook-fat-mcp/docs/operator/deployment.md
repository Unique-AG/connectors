...
```bash
helm install outlook-fat-mcp oci://ghcr.io/unique-ag/helm-charts/outlook-fat-mcp \
...
```bash
helm upgrade outlook-fat-mcp oci://ghcr.io/unique-ag/helm-charts/outlook-fat-mcp \
  --namespace outlook-fat-mcp \
  --values values.yaml
```
...
kind: Secret
metadata:
  name: outlook-fat-mcp-secrets
  namespace: outlook-fat-mcp
type: Opaque
stringData:
  DATABASE_URL: "postgresql://user:password@host:5432/outlook_fat_mcp"
  AMQP_URL: "amqp://user:password@rabbitmq:5672/outlook-fat-mcp"
...
```bash
kubectl exec -it deploy/outlook-fat-mcp -- pnpm run db:migrate
```
...
- **Entra Application**: `deploy/terraform/azure/outlook-fat-mcp-entra-application/`
- **Secrets Management**: `deploy/terraform/azure/outlook-fat-mcp-secrets/`
