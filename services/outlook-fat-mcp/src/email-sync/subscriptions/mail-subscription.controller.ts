...
import { RabbitSubscribe, defaultNackErrorHandler, RabbitPayload } from '@golevelup/nestjs-rabbitmq';
import { MAIN_EXCHANGE, DEAD_EXCHANGE } from '~/amqp/amqp.constants';
import { wrapErrorHandlerOTEL } from '~/amqp/amqp.utils';
  ...
  export class MailSubscriptionController {
    ...
    @RabbitSubscribe({
      ...
      queue: 'unique.outlook-mcp.transcript.lifecycle-notifications',
      routingKey: ['unique.outlook-mcp.transcript.lifecycle-notification.*'],
    ...
    public async onLifecycleNotification(
      ...
      switch (event.type) {
        case 'unique.outlook-mcp.mail.lifecycle-notification.subscription-requested': {
          return this.subscriptionCreate.subscribe(event.userProfileId);
        }
        case 'unique.outlook-mcp.mail.lifecycle-notification.subscription-removed': {
          return this.subscriptionRemove.remove(event.subscriptionId);
        }
        case 'unique.outlook-mcp.mail.lifecycle-notification.reauthorization-required': {
