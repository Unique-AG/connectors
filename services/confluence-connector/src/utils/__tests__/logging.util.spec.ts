...
describe('smear', () => {
  ...
  it('replaces middle characters with asterisks for longer strings', () => {
    expect(smear('password')).toBe('****word');
    expect(smear('mySecret123')).toBe('*******t123');
    expect(smear('verylongstring')).toBe('**********ring');
  });

  it('preserves non-alphanumeric characters in the smeared portion', () => {
    expect(smear('admin@acme.com')).toBe('*****@****.com');
  });
