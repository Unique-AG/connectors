"""Failed-login throttling for the hosted login form.

`POST /login` forwards a submitted username and password to With Intelligence and reports
whether it authenticated. Anyone who can start an OAuth flow can mint a valid `request_id`, so
without a limit that endpoint is a credential-testing oracle against a third party.
...
from with_intelligence_mcp.db import LoginAttempt, read_session, transaction
