...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(["client_id"], ["mcp_auth_oauth_clients.client_id"]),
        sa.PrimaryKeyConstraint("code"),
    )
    op.create_index(
        op.f("ix_mcp_auth_authorization_codes_subject"),
        "mcp_auth_authorization_codes",
        ["subject"],
        unique=False,
    )
    op.create_table(
        "mcp_auth_oauth_tokens",
        ...
        sa.ForeignKeyConstraint(["client_id"], ["mcp_auth_oauth_clients.client_id"]),
        sa.ForeignKeyConstraint(["rotated_from"], ["mcp_auth_oauth_tokens.id"]),
        sa.PrimaryKeyConstraint("id"),
    )
    op.create_index(
        op.f("ix_mcp_auth_oauth_tokens_access_token_hash"),
        "mcp_auth_oauth_tokens",
        ["access_token_hash"],
        unique=True,
    )
    op.create_index(
        op.f("ix_mcp_auth_oauth_tokens_family_id"),
        "mcp_auth_oauth_tokens",
        ["family_id"],
        unique=False,
    )
    op.create_index(
        op.f("ix_mcp_auth_oauth_tokens_refresh_token_hash"),
        "mcp_auth_oauth_tokens",
        ["refresh_token_hash"],
        unique=True,
    )
    op.create_index(
        op.f("ix_mcp_auth_oauth_tokens_subject"),
        "mcp_auth_oauth_tokens",
        ["subject"],
        unique=False,
    )
    ...
    op.create_index(
        "ix_mcp_auth_login_attempts_attempted_at",
        "mcp_auth_login_attempts",
        ["attempted_at"],
    )
...
def downgrade() -> None:
    op.drop_index("ix_mcp_auth_login_attempts_attempted_at", table_name="mcp_auth_login_attempts")
    op.drop_index(
        "ix_mcp_auth_login_attempts_username_attempted_at",
        table_name="mcp_auth_login_attempts",
    )
    op.drop_table("mcp_auth_login_attempts")
    op.drop_table("mcp_auth_pending_authorizations")
    op.drop_index(op.f("ix_mcp_auth_oauth_tokens_subject"), table_name="mcp_auth_oauth_tokens")
    op.drop_index(
        op.f("ix_mcp_auth_oauth_tokens_refresh_token_hash"),
        table_name="mcp_auth_oauth_tokens",
    )
    op.drop_index(op.f("ix_mcp_auth_oauth_tokens_family_id"), table_name="mcp_auth_oauth_tokens")
    op.drop_index(
        op.f("ix_mcp_auth_oauth_tokens_access_token_hash"),
        table_name="mcp_auth_oauth_tokens",
    )
    op.drop_table("mcp_auth_oauth_tokens")
    op.drop_index(
        op.f("ix_mcp_auth_authorization_codes_subject"),
        table_name="mcp_auth_authorization_codes",
    )
    op.drop_table("mcp_auth_authorization_codes")
    op.drop_table("mcp_auth_oauth_clients")
    op.drop_index(
        op.f("ix_with_intelligence_sessions_wi_username"),
