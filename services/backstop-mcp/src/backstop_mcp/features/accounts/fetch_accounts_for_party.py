"""List Backstop accounts belonging to one owning party.
...
`/organizations/{id}` exposes an `accounts` subcollection. By-party listing therefore walks the
whole of `/accounts` and keeps rows the party owns: the `relationships.owner` linkage id first,
since that needs no side-load, then the projected owner id, which is
`specificResource.resourceId` for an organization owner. Matching only the linkage would drop
exactly the rows whose owner id the tool goes on to echo. Open means the `closedDate` key is
absent.

The walk asks for `fields=` and pages in parallel, which together is what makes it affordable:
unfiltered, an `accounts` resource is ~9.6 KiB, ~90% of it the `links` boilerplate of 30
relationships nothing here reads. `fields=` drops the whole `relationships` block — except for
the relationships named in `include=`, which keep their `data` linkage, so `_owns` still sees
...
async def fetch_accounts_for_party(
    ...
    *,
    ...
    page = await client.paginate(
        ...
        params={"include": _INCLUDE_BY_PARTY, "fields": ACCOUNT_LISTING_FIELDS},
        max_records=None,
