"""Double-submit CSRF protection for the hosted Backstop login form.

`POST /backstop/login` takes a username and personal API token and, on success, mints an
...
deliberately does *not* try to prove is which Backstop account the browser belongs to — see
...
_COOKIE_PREFIX = "backstop_login_csrf_"
_TOKEN_BYTES = 32
