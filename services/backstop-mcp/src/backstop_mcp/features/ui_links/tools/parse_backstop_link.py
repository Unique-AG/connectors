...
from backstop_mcp.features.ui_links.dependencies import get_parse_entity_link_util_factory
...
async def parse_backstop_link(
    ...
    a party id, and a task URL does not load via `get_tasks_for_party`. An ActivitySearch
    fragment yields the related party id, not an activity id. Unrecognized URLs return
    `status=unrecognized`; do not invent a target. A configured UI host that differs from
    the pasted host sets `host_mismatch=true` but the URL is still parsed.

    Call like: {"url": "https://tenant.example.test/backstop/crm/ManageOrganization.action?party_id=<id>"}
    """
    return parse_entity_link_util.run(url=url)
