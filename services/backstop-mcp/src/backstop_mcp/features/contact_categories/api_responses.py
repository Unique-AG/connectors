...
__all__ = ["ContactCategoryAttributes"]


class ContactCategoryAttributes(BaseModel):
    """Wire shape for `contact-categories` attributes (subset we publish).
    ...
    plus the drop in `ContactCategoryDto.from_resource` keep one bad row from costing the rest.
    """
