...
from backstop_mcp.db.models import AuthorizationCode, OAuthToken, PendingAuthorization
...
async def purge_expired_auth_rows(
    session_factory: async_sessionmaker[AsyncSession], *, token_retention: timedelta
) -> None:
...
async def _sweep_forever(
    ...
    while True:
        try:
            await purge_expired_auth_rows(session_factory, token_retention=config.token_retention)
        except Exception:
