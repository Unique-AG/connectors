...
import type { UniquePublicSdkOptions } from '../unique-public-sdk.options';
...
export class UniqueContentService {
  ...
  public constructor(
    @Inject(UNIQUE_PUBLIC_FETCH) private readonly fetch: FetchFn,
    @Inject(UNIQUE_PUBLIC_SDK_OPTIONS) private readonly options: UniquePublicSdkOptions,
  ...
  public async uploadToStorage(
    ...
    let effectiveUrl = writeUrl;

    if (this.options.storageInternalBaseUrl) {
      const original = new URL(writeUrl);
      const internal = new URL(this.options.storageInternalBaseUrl);
      original.protocol = internal.protocol;
      original.host = internal.host;
      effectiveUrl = original.toString();
    }
