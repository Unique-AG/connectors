...
const parseTokenJson = Effect.fn('OnBehalfOfAuth.parseTokenJson')(
  ...
  Effect.annotateLogs({ service: 'OnBehalfOfAuth', method: 'parseTokenJson' }),
);
...
const exchangeOboToken = Effect.fn('OnBehalfOfAuth.exchangeOboToken')(
      ...
      Effect.timeoutOrElse({
        ...
        orElse: () =>
          Effect.fail(
            new AuthenticationFailedError({
              reason: 'unknown',
              message: 'OBO token acquisition timed out',
              correlationId: undefined,
            }),
          ),
      }),
  ...
  Effect.annotateLogs({ service: 'OnBehalfOfAuth', method: 'exchangeOboToken' }),
);
...
const acquireOboToken = Effect.fn('OnBehalfOfAuth.acquireOboToken')(
  ...
  Effect.annotateLogs({ service: 'OnBehalfOfAuth', method: 'acquireOboToken' }),
);
...
const getCachedAccounts = Effect.fn('OnBehalfOfAuth.getCachedAccounts')(
  ...
  Effect.annotateLogs({ service: 'OnBehalfOfAuth', method: 'getCachedAccounts' }),
);
...
export const OnBehalfOfAuthLive = <_P extends string = string>() =>
  Layer.effect(
    OnBehalfOfAuth,
    Effect.gen(function* () {
    ...
    }).pipe(Effect.withSpan('OnBehalfOfAuthLive.initialize')),
  );
