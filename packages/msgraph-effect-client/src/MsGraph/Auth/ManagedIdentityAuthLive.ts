...
import { isTokenExpired, REFRESH_BUFFER_MS } from './TokenCache';
...
const parseImdsJson = Effect.fn('ManagedIdentityAuth.parseImdsJson')(
  ...
  Effect.annotateLogs({ service: 'ManagedIdentityAuth', method: 'parseImdsJson' }),
);
...
const acquireFromImds = Effect.fn('ManagedIdentityAuth.acquireFromImds')(
    ...
    if (!primaryScope) {
      return yield* Effect.fail(
        new AuthenticationFailedError({
          ...
          message: 'At least one scope must be provided for managed identity token acquisition',
          correlationId: undefined,
        }),
      );
    }
      ...
      Effect.timeoutOrElse({
        ...
        orElse: () =>
          Effect.fail(
            new AuthenticationFailedError({
              reason: 'unknown',
              message: 'IMDS token acquisition timed out',
              correlationId: undefined,
            }),
          ),
      }),
  ...
  Effect.annotateLogs({ service: 'ManagedIdentityAuth', method: 'acquireFromImds' }),
);
...
const acquireManagedIdentityToken = Effect.fn('ManagedIdentityAuth.acquireManagedIdentityToken')(
  ...
  Effect.annotateLogs({ service: 'ManagedIdentityAuth', method: 'acquireManagedIdentityToken' }),
);
...
const makeService = (
  ...
  return {
    ...
    removeCachedAccount: (_accountId: string) => Ref.set(cachedRef, Option.none()),
  };
};

// Generic parameter _P is intentional type branding used by external callers (e.g. MsGraphClient)
export const ManagedIdentityAuthLive = <_P extends string = string>() =>
  Layer.effect(
    ManagedIdentityAuth,
    Effect.gen(function* () {
      const config = yield* ManagedIdentityAuthConfig;
      const cachedRef = yield* Ref.make<Option.Option<AccessTokenInfo>>(Option.none());

      return ManagedIdentityAuth.of(
        makeService({ scopes: config.scopes, clientId: config.clientId }, cachedRef),
      );
    }).pipe(Effect.withSpan('ManagedIdentityAuthLive.initialize')),
  );
