...
export interface TokenCachePlugin {
  readonly beforeCacheAccess: (cache: TokenCacheContext) => Effect.Effect<void, never>;
  readonly afterCacheAccess: (cache: TokenCacheContext) => Effect.Effect<void, never>;
}
...
export class TokenCacheStateRef extends ServiceMap.Service<
  TokenCacheStateRef,
  Ref.Ref<TokenCacheState>
>()('MsGraph/Auth/TokenCacheStateRef') {}

export const makeScopesKey = (accountId: string, scopes: ReadonlyArray<string>): string =>
  `${accountId}:${[...scopes].sort().join(' ')}`;
...
export const makeTokenCacheContext = (stateRef: Ref.Ref<TokenCacheState>): TokenCacheContext => {
  ...
  return {
    ...
    deserialize: (data: string) => {
      ...
      const accounts = new Map<string, AccountCacheEntry>(
        Object.entries(parsed.accounts ?? {}).map(([k, v]) => [k, v as AccountCacheEntry]),
      );
      hasChanged = true;
      stateRef.ref.current = { tokens, accounts };
    },
  };
};
