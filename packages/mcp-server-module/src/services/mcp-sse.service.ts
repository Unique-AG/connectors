...
export class McpSseService {
   ...
   */
  public async createSseConnection(
    rawReq: any,
    rawRes: any,
   ...
   */
  public async handleMessage(rawReq: any, rawRes: any, body: unknown): Promise<any> {
    const adapter = HttpAdapterFactory.getAdapter(rawReq, rawRes);
    ...
    const executor = await this.moduleRef.resolve(McpExecutorService, contextId);
