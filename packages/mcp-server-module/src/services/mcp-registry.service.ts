...
import { Injectable, InjectionToken, Logger, OnApplicationBootstrap } from '@nestjs/common';
...
export class McpRegistryService implements OnApplicationBootstrap {
  ...
  private discoveredToolsByMcpModuleId: Map<string, DiscoveredTool<any>[]> = new Map();
  ...
  private discoverTools() {
    const getImportedMcpModules = (module: Module) =>
      Array.from(module.imports).filter((m) => (m.instance as any).__isMcpModule);

    const pairs = Array.from(this.modulesContainer.values())
      .map((module): [Module, Module[]] => [module, getImportedMcpModules(module)])
    ...
    for (const [rootModule, mcpModules] of pairs) {
      ...
      for (const mcpModule of mcpModules) {
        const mcpModuleId = mcpModule.getProviderByKey<string>('MCP_MODULE_ID')?.instance;
   ...
   */
  private discoverToolsForModuleSubtree(mcpModuleId: string, modules: Module[]) {
    ...
    const allInstances = [...providers, ...controllers]
      .filter(
        (wrapper) =>
          wrapper.instance && typeof wrapper.instance === 'object' && wrapper.instance !== null,
      )
      .map((wrapper) => ({
  ...
  private addDiscovery<T>(
    ...
    if (!(metadata as any).name) {
      (metadata as any).name = methodName;
    }
   ...
   */
  public getTools(mcpModuleId: string): DiscoveredTool<ToolMetadata>[] {
    return (
      this.discoveredToolsByMcpModuleId.get(mcpModuleId)?.filter((tool) => tool.type === 'tool') ??
      []
    );
  }
   ...
   * Find a tool by name
   */
  public findTool(mcpModuleId: string, name: string): DiscoveredTool<ToolMetadata> | undefined {
   ...
   */
  public getResources(mcpModuleId: string): DiscoveredTool<ResourceMetadata>[] {
    return (
   ...
   */
  public getResourceTemplates(mcpModuleId: string): DiscoveredTool<ResourceTemplateMetadata>[] {
    return (
      this.discoveredToolsByMcpModuleId
        .get(mcpModuleId)
   ...
   */
  public getPrompts(mcpModuleId: string): DiscoveredTool<PromptMetadata>[] {
    return (
      this.discoveredToolsByMcpModuleId
        .get(mcpModuleId)
   ...
   */
  public findPrompt(mcpModuleId: string, name: string): DiscoveredTool<PromptMetadata> | undefined {
