...
export class McpStreamableHttpService {
   ...
   */
  public async createStatelessServer(rawReq: any): Promise<{
    server: McpServer;
    ...
    const executor = await this.moduleRef.resolve(McpExecutorService, contextId, { strict: true });
   ...
   */
  public async handlePostRequest(req: any, res: any, body: unknown): Promise<void> {
   ...
   */
  public async handleStatelessRequest(req: any, res: HttpResponse, body: unknown): Promise<void> {
    this.logger.debug(
      `Handling stateless MCP request at ${req.url} with body: ${JSON.stringify(body)}`,
    );
   ...
   */
  public async handleDeleteRequest(req: any, res: any): Promise<void> {
    // Get the appropriate HTTP adapter for the request/response
    const adapter = HttpAdapterFactory.getAdapter(req, res);
    const adaptedReq = adapter.adaptRequest(req);
    const adaptedRes = adapter.adaptResponse(res);
  ...
  private isInitializeRequest(body: unknown): boolean {
    if (Array.isArray(body)) {
      return body.some(
        (msg) =>
          typeof msg === 'object' && msg !== null && 'method' in msg && msg.method === 'initialize',
      );
    }
    return (
