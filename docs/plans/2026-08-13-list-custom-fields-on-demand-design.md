...
**Service** — one process-wide catalog + `TimedGate` + `asyncio.Lock`. TTL is
`BACKSTOP_CUSTOM_FIELD_SCHEMA_TTL_MINUTES` (default 60, max 24h; values above the cap are
clamped). `get(client, refresh=False)`: if fresh and not refresh, return the list; else fetch,
replace, stamp. Concurrent waiters share the in-flight fetch. No Postgres, no per-subject map,
no overrides.
...
`BACKSTOP_SERVICE_*` and `BACKSTOP_CUSTOM_FIELD_OVERRIDES` from config. Keep
`BACKSTOP_CUSTOM_FIELD_SCHEMA_TTL_MINUTES` (default 60, max 24h). `/ready` checks Postgres
only. `get_person` / `get_organization` lose `glossary_meta` and the “call list_custom_fields
when truncated” blurb; they point at `list_custom_fields` with the matching type when the model
needs field names.
...
unknown Bean is dropped; `selectOptions` is copied from attributes, including when Backstop
inlines the options as an object under a collection key (`options`, `entries`, `data`, …); no
`/lov-entries` or `include=lovSet`. Overrides are not applied because they no longer exist.

**Removals** — middleware tests go away with the middleware. App/config tests no longer expect
`BACKSTOP_SERVICE_*`, override JSON, or `/ready`'s `custom_field_schema` check. Schema TTL stays
as `BACKSTOP_CUSTOM_FIELD_SCHEMA_TTL_MINUTES`. `get_person` / `get_organization` no longer
declare `glossary_meta`. Snapshot/store/override/glossary/LOV tests go with those modules.
...
- **`/lov-entries` and `?include=lovSet`** — dropdowns use `attributes.selectOptions` only
  (list or object-shaped collection). Fields whose options live only on the LOV relationship
  will have an empty `selectOptions` list.
...
- **Configurable page size for this fetch** — `page[limit]=1000` is hardcoded.
  Global `BACKSTOP_DEFAULT_PAGE_SIZE` is unchanged. TTL is configurable
  (`BACKSTOP_CUSTOM_FIELD_SCHEMA_TTL_MINUTES`, default 60, max 24h).
...
2. **Drop service account and warmup; keep a capped schema TTL** — Remove
   `BACKSTOP_SERVICE_USERNAME` / `BACKSTOP_SERVICE_API_TOKEN`, boot `warmup_lifespan`, and the
   `/ready` `custom_field_schema` check. Keep `BACKSTOP_CUSTOM_FIELD_SCHEMA_TTL_MINUTES` (default
   60, max 24h, clamp above the cap). Update `.env.example` and README.
...
7. **Slim `CustomFieldsService` to a process-wide catalog** — One in-memory list, `TimedGate`
   from `BACKSTOP_CUSTOM_FIELD_SCHEMA_TTL_MINUTES` (default 1 hour, max 24h), single-flight
   lock, `refresh` bypasses TTL. Failed fetch with a cache keeps stale; cold failure raises.
   Delete subject-keyed snapshot logic and unused `resolve_field` / `read_custom_field_value` /
   index helpers that depended on it.
